...
function Gallery() {
  const media = [
    ...
    { type: 'image', src: '/w2copy.jpeg', alt: 'Construction 9' },
    { type: 'image', src: '/w4copy.jpeg', alt: 'Construction 9' },
    { type: 'image', src: '/w5copy.jpeg', alt: 'Construction 9' },
  ...
  useEffect(() => {
    // If navigated with #videos, open videos filter and scroll so the
    // videos section sits at the very top of the viewport (account for fixed nav).
    if (typeof window !== 'undefined' && window.location.hash === '#videos') {
      ...
      setTimeout(() => {
        ...
        const scrollTo = Math.max(0, top - offset - 8)
  ...
  return (
    <>
      <Navbar />
      <main className={`min-h-screen pt-20 ${selectedVideo ? 'filter blur-sm' : ''} bg-gray-100 dark:bg-gray-900`}>
        <div className="max-w-7xl mx-auto px-4 py-8">
          <h1 className="text-2xl font-bold">Gallery</h1>
          <p className="mt-4 text-gray-600">A mix of images and short reels from completed projects.</p>
          ...
          <div className="mt-6 flex ">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full ${filter === 'images' ? 'bg-white text-gray-900 shadow' : 'text-gray-600 dark:text-gray-300'}`}
            >
              Images
            </button>
            <button
              onClick={() => setFilter('videos')}
              className={`ml-1 px-4 py-2 rounded-full ${filter === 'videos' ? 'bg-white text-gray-900 shadow' : 'text-gray-600 dark:text-gray-300'}`}
            >
              Videos
            </button>
            </div>
          ...
          <section className="mt-8">
            ...
            <div>
              <div className="mb-4 flex items-center justify-between">
                ...
                <p className="text-sm text-gray-500">Photos from completed projects</p>
              </div>
                ...
                {media.filter((m) => m.type === 'image').map((m, i) => (
                  <div key={i} className="rounded-lg overflow-hidden bg-gray-100 shadow-sm">
                    <div className="relative h-48 w-full">
                      <Image src={m.src} alt={m.alt || 'image'} fill className="object-cover" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div id="videos" ref={videosRef}>
              <div className="mt-8 mb-4 flex items-center justify-between">
                ...
                <p className="text-sm text-gray-500">Short reels and walkthroughs</p>
              </div>
